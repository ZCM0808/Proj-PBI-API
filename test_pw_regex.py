...
test_name = 'Proj-PBI-API UI e2e tests › 溢出防御 (Overflow Defense): 动作按钮组绝对不能跑到右侧面板之外'

# Escape only regex metacharacters, but leave spaces alone!
def escape_playwright_regex(text):
    return re.sub(r'([()[\]{}.?*+^$|])', r'\\\1', text)

pattern = escape_playwright_regex(test_name)
