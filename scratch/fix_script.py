...
old_regex = re.compile(r'window\.executeDatasetStep2 = async function\(btn\) \{.*?\};\n\nwindow\.executeExportDataset = async function\(\) \{.*?\};', re.DOTALL)

new_code = r"""window.executeDatasetStep2 = async function(btn) {
...
if not old_regex.search(js):
    print("Failed to find executeDatasetStep2")
else:
    js = old_regex.sub(new_code, js, count=1)
