        ...
        renderUserAssetsMatrix() {
            ...
            if (!hasSelectedWs) {
                ...
                const modelItems = [
                    ...
                    { id: 'model_gac_ols', name: 'GAC / OLS (表与列细粒度对象安全)', desc: isPrivileged ? '【特权】工作区管理员特权穿透，免除语义模型敏感表与度量值字段的 OLS/GAC 掩蔽限制' : (user?.state?.olsEnabled ? '【限制】受敏感字段 OLS/GAC 细粒度安全约束，受保护的高密字段已被动态掩蔽 (Masked)' : '【可以】已获模型全量表与字段 GAC 访问权限，所有敏感维度与指标字段完整可见'), statusClass: isPrivileged ? 'bypassed' : (user?.state?.olsEnabled ? 'warn' : 'enabled'), statusText: isPrivileged ? '⚡ ADMIN BYPASS' : (user?.state?.olsEnabled ? '🔒 OLS MASKED' : '✅ CAN ACCESS ALL'), badge: 'OLS' },
            ...
            if (!hasSelectedWs) {
                ...
                connItems.push(
                    ...
                    { id: 'conn_gac_perm', name: 'GAC PERMISSION (数据源细粒度访问权限)', desc: isPrivileged ? '【特权】工作区管理员特权穿透，直接拥有该连接最高 GAC(Granular Access Control) 细粒度物理直连与抽取权限' : (hasDataConn ? '【可以】已获官方数据源 GAC 细粒度授权，允许直接复用此连接凭据执行数据查询与抽取' : '【禁止】未被分配 GAC 细粒度权限，无法通过此连接访问底层物理数据库'), statusClass: isPrivileged ? 'bypassed' : (hasDataConn ? 'enabled' : 'disabled'), statusText: isPrivileged ? '⚡ ADMIN BYPASS' : (hasDataConn ? '✅ CAN ACCESS' : '❌ CANNOT ACCESS'), badge: 'GAC' },
                    { id: 'conn_gac_mashup', name: 'GAC MASHUP GATE (跨源数据混合转换门禁)', desc: isPrivileged ? '【特权】工作区管理员直通，豁免多数据源 Mashup 细粒度门禁限制，可自由混合处理多源数据' : (hasDataConn && !user?.state?.isInStrictMode ? '【可以】跨源安全门禁放行，允许在 Power Query 与 DirectQuery 中将此连接与其它数据源关联合并' : '【禁止】触发 GAC 跨源安全隔离门禁，严格模式下禁止跨数据源混合关联处理'), statusClass: isPrivileged ? 'bypassed' : (hasDataConn && !user?.state?.isInStrictMode ? 'enabled' : 'disabled'), statusText: isPrivileged ? '⚡ ADMIN BYPASS' : (hasDataConn && !user?.state?.isInStrictMode ? '✅ CAN MASHUP' : '❌ CANNOT MASHUP'), badge: 'MASHUP' },
