    ...
    class PermissionBlueprintEngine {
        ...
        renderUserAssetsMatrix() {
            ...
            const tenantItems = [
                ...
                { id: 'tenant_export', name: 'EXPORT DATA (明细数据导出策略)', desc: user ? '租户全局策略放行，允许将报表与模型数据导出至本地 Excel/CSV' : '【等待配置】需选定具体登录主体后生效策略', statusClass: user ? 'enabled' : 'disabled', statusText: user ? '✅ CAN EXPORT' : '❌ CANNOT EXPORT', badge: 'EXPORT' },
                { id: 'tenant_web_modeling', name: 'WEB MODELING (浏览器在线建模)', desc: user?.state?.tenantAllowWebModeling ? '租户策略允许在浏览器端直接设计、编辑语义模型架构与度量值' : '租户策略禁用网页在线建模，只能通过客户端工具操作', statusClass: user?.state?.tenantAllowWebModeling ? 'enabled' : 'disabled', statusText: user?.state?.tenantAllowWebModeling ? '✅ CAN MODEL' : '❌ CANNOT MODEL', badge: 'WEB MODEL' },
                { id: 'tenant_xmla', name: 'XMLA ENDPOINT (终结点全局读写)', desc: '终结点已开启读写，允许 SSMS、DAX Studio 与 Tabular Editor 跨客户端直连', statusClass: 'enabled', statusText: '✅ CAN CONNECT', badge: 'XMLA' },
                { id: 'tenant_external', name: 'EXTERNAL SHARING (跨组织外部共享)', desc: user ? (isGuest ? '当前属于外部访客账号，默认受限禁止跨租户二次外发共享' : '租户策略放行组织外部跨域报告共享') : '【等待配置】需选定用户主体后推导策略', statusClass: isGuest ? 'disabled' : (user ? 'enabled' : 'disabled'), statusText: isGuest ? '❌ CANNOT SHARE' : (user ? '✅ CAN SHARE' : '⚠️ WAITING'), badge: 'EXTERNAL' },
            ...
            if (!hasSelectedWs) {
                ...
                const wsItems = [
                    ...
                    { id: 'ws_members', name: 'MANAGE PERMISSIONS (管理与成员委派)', desc: isAdmin ? '拥有最高管理权，可向组织成员分配、修改或撤销工作区各级角色' : (isMember ? '仅允许向他人授予 Viewer(查看者) 角色，无法分配更高权限' : '无成员管理权限，禁止变更工作区成员名单与权限'), statusClass: isAdmin ? 'enabled' : (isMember ? 'warn' : 'disabled'), statusText: isAdmin ? '✅ CAN MANAGE' : (isMember ? '⚠️ CAN INVITE VIEWERS' : '❌ CANNOT MANAGE'), badge: 'PERMISSIONS' },
                    { id: 'ws_edit', name: 'CREATE & EDIT ASSETS (资产协同增删改)', desc: isPrivileged ? '拥有资产编辑特权，允许新建、修改、重命名或删除模型与报表' : '当前为 Viewer 只读角色，禁止修改或新增工作区任何资产', statusClass: isPrivileged ? 'enabled' : 'disabled', statusText: isPrivileged ? '✅ CAN EDIT' : '❌ CANNOT EDIT', badge: 'ASSETS' },
                    { id: 'ws_app', name: 'PUBLISH APP (组织应用打包发布)', desc: (isAdmin || isMember) ? '允许将该工作区报表打包发布或更新为企业级应用程序 (App)' : '仅 Admin/Member 角色具备组织应用发布与受众打包权限', statusClass: (isAdmin || isMember) ? 'enabled' : 'disabled', statusText: (isAdmin || isMember) ? '✅ CAN PUBLISH' : '❌ CANNOT PUBLISH', badge: 'APP' },
                    { id: 'ws_capacity', name: 'FABRIC CAPACITY (算力容量绑定)', desc: '挂载企业专用容量 (Fabric F64)，享有独立计算算力与大模型加速', statusClass: 'enabled', statusText: '⚡ CAN ACCESS', badge: 'CAPACITY' },
            ...
            if (!hasSelectedWs) {
                ...
                const modelItems = [
                    ...
                    { id: 'model_read', name: 'READ (模型直接读取权限)', desc: canReadModel ? '执行 DAX 查询与模型基础刷新，下游报表正常取数渲染' : '无 READ 权限，DAX 查询将被 403 阻断，报表将拒绝加载', statusClass: canReadModel ? 'enabled' : 'disabled', statusText: canReadModel ? '✅ CAN READ' : '❌ CANNOT READ', badge: 'READ' },
                    { id: 'model_build', name: 'BUILD (衍生构建与自助探索)', desc: canBuild ? '允许以该模型为基础使用 Excel 透视分析、新建独立衍生报表' : '无 BUILD 权限，无法新建下游衍生报表或在 Excel 中连接探索', statusClass: canBuild ? 'enabled' : 'disabled', statusText: canBuild ? '✅ CAN BUILD' : '❌ CANNOT BUILD', badge: 'BUILD' },
                    { id: 'model_write', name: 'WRITE (架构与度量值写回)', desc: isPrivileged ? '通过 XMLA 端点或浏览器在线修改表结构、新建度量值与关系模型' : '非 Admin/Member/Contributor 角色，禁止写回模型架构或修改度量值', statusClass: isPrivileged ? 'enabled' : 'disabled', statusText: isPrivileged ? '✅ CAN WRITE' : '❌ CANNOT WRITE', badge: 'WRITE' },
                    { id: 'model_gac_ols', name: 'GAC / OLS (表与列细粒度对象安全)', desc: isPrivileged ? '工作区管理员特权穿透，免除语义模型敏感表与度量值字段的 OLS/GAC 掩蔽限制' : (user?.state?.olsEnabled ? '受敏感字段 OLS/GAC 细粒度安全约束，受保护的高密字段已被动态掩蔽 (Masked)' : '已获模型全量表与字段 GAC 访问权限，所有敏感维度与指标字段完整可见'), statusClass: isPrivileged ? 'bypassed' : (user?.state?.olsEnabled ? 'warn' : 'enabled'), statusText: isPrivileged ? '⚡ ADMIN BYPASS' : (user?.state?.olsEnabled ? '🔒 OLS MASKED' : '✅ CAN ACCESS ALL'), badge: 'OLS' },
                    { id: 'model_rls', name: 'RLS (行级数据安全过滤)', desc: isPrivileged ? '工作区管理员特权穿透，直接跳过所有 DAX 行级安全过滤规则' : '受 DAX 角色策略约束，仅能查看授权给当前身份的切片行数据', statusClass: isPrivileged ? 'bypassed' : 'warn', statusText: isPrivileged ? '⚡ ADMIN BYPASS' : '🔒 RLS RESTRICTED', badge: 'RLS' },
                    { id: 'model_reshare', name: 'RESHARE (向第三方重新共享)', desc: (isAdmin || isMember) ? '允许将该具体语义模型的访问权限二次授权给其他组织成员' : '无 RESHARE 权限，禁止向第三方组织成员分发或再授权该模型', statusClass: (isAdmin || isMember) ? 'enabled' : 'disabled', statusText: (isAdmin || isMember) ? '✅ CAN RESHARE' : '❌ CANNOT RESHARE', badge: 'RESHARE' }
                ];
            ...
            if (!hasSelectedWs) {
                ...
                const reportItems = [
                    ...
                    { id: 'report_view', name: 'VIEW & INTERACT (报表在线交互)', desc: '在线访问报表页面、切片器联动与图表多维钻取浏览', statusClass: 'enabled', statusText: '✅ CAN VIEW', badge: 'VIEW' },
                    { id: 'report_edit', name: 'EDIT VISUALS (视觉对象在线编辑)', desc: canEditReport ? '在线修改报表图表、调整页面布局与另存副本' : '未被授予编辑权限，报表处于纯只读交互模式，无法修改布局', statusClass: canEditReport ? 'enabled' : 'disabled', statusText: canEditReport ? '✅ CAN EDIT' : '❌ CANNOT EDIT', badge: 'EDIT' },
                    { id: 'report_export', name: 'EXPORT DATA (底层明细数据导出)', desc: canExportUnderlying ? '允许导出底层原始颗粒度数据明细至本地 Excel/CSV' : '缺少 BUILD 权限或受租户策略限制，仅允许导出带格式汇总数据', statusClass: canExportUnderlying ? 'enabled' : 'warn', statusText: canExportUnderlying ? '✅ CAN EXPORT' : '⚠️ SUMMARY ONLY', badge: 'EXPORT' },
                    { id: 'report_sub', name: 'SUBSCRIBE & ALERT (订阅与数据警报)', desc: '设置报表关键 KPI 阈值自动化警报及定时邮件快照推送', statusClass: 'enabled', statusText: '✅ CAN SUBSCRIBE', badge: 'SUBSCRIBE' },
                    { id: 'report_share', name: 'SHARE REPORT (报表安全链接共享)', desc: (isAdmin || isMember) ? '生成组织安全共享链接向授权受众分发报表' : '仅 Admin/Member 具备报表受众共享与链接分发权限', statusClass: (isAdmin || isMember) ? 'enabled' : 'disabled', statusText: (isAdmin || isMember) ? '✅ CAN SHARE' : '❌ CANNOT SHARE', badge: 'SHARE' }
                ];
            ...
            if (!hasSelectedWs) {
                ...
                connItems.push(
                    { id: 'conn_user_perm', name: 'CONNECTION USER (连接凭据使用权)', desc: hasDataConn ? '具备 Connection User 授权，模型在刷新与 DirectQuery 取数时可复用此凭据' : '未被分配 Connection User 角色，无法调用或复用该连接凭据', statusClass: hasDataConn ? 'enabled' : 'disabled', statusText: hasDataConn ? '✅ CAN USE' : '❌ CANNOT USE', badge: 'CREDENTIALS' },
                    { id: 'conn_gac_perm', name: 'GAC PERMISSION (数据源细粒度访问权限)', desc: isPrivileged ? '工作区管理员特权穿透，直接拥有该连接最高 GAC(Granular Access Control) 细粒度物理直连与抽取权限' : (hasDataConn ? '已获官方数据源 GAC 细粒度授权，允许直接复用此连接凭据执行数据查询与抽取' : '未被分配 GAC 细粒度权限，无法通过此连接访问底层物理数据库'), statusClass: isPrivileged ? 'bypassed' : (hasDataConn ? 'enabled' : 'disabled'), statusText: isPrivileged ? '⚡ ADMIN BYPASS' : (hasDataConn ? '✅ CAN ACCESS' : '❌ CANNOT ACCESS'), badge: 'GAC' },
                    { id: 'conn_gac_mashup', name: 'GAC MASHUP GATE (跨源数据混合转换门禁)', desc: isPrivileged ? '工作区管理员直通，豁免多数据源 Mashup 细粒度门禁限制，可自由混合处理多源数据' : (hasDataConn && !user?.state?.isInStrictMode ? '跨源安全门禁放行，允许在 Power Query 与 DirectQuery 中将此连接与其它数据源关联合并' : '触发 GAC 跨源安全隔离门禁，严格模式下禁止跨数据源混合关联处理'), statusClass: isPrivileged ? 'bypassed' : (hasDataConn && !user?.state?.isInStrictMode ? 'enabled' : 'disabled'), statusText: isPrivileged ? '⚡ ADMIN BYPASS' : (hasDataConn && !user?.state?.isInStrictMode ? '✅ CAN MASHUP' : '❌ CANNOT MASHUP'), badge: 'MASHUP' },
                    { id: 'conn_gw', name: gwItemName, desc: gwItemDesc, statusClass: gwItemStatusClass, statusText: gwItemStatusText, badge: gwItemBadge },
                    { id: 'conn_sso', name: 'DIRECTQUERY SSO (单点登录身份委派)', desc: 'DirectQuery 运行时使用当前用户 Entra ID 身份穿透鉴权直连底层数据库', statusClass: 'enabled', statusText: '✅ CAN DELEGATE', badge: 'SSO' },
                    { id: 'conn_refresh', name: 'SCHEDULED REFRESH (计划刷新调度)', desc: isPrivileged ? '允许配置自动化计划刷新调度并随时手动触发微批次数据抽取' : '仅 Admin/Member/Contributor 具备计划刷新配置与手动触发权限', statusClass: isPrivileged ? 'enabled' : 'disabled', statusText: isPrivileged ? '✅ CAN REFRESH' : '❌ CANNOT REFRESH', badge: 'REFRESH' }
                );
            ...
            const pipelineItems = [
                ...
                { id: 'pipeline_deploy', name: 'STAGE DEPLOYMENT (阶段流转部署)', desc: isPipelineAdmin ? '允许将开发阶段的模型与报表一键晋升部署至测试 (Test) 或生产 (Prod) 环境' : '尚未绑定专用管道或缺少部署者权限，无法执行阶段流转', statusClass: isPipelineAdmin ? 'enabled' : 'warn', statusText: isPipelineAdmin ? '✅ CAN DEPLOY' : '⚠️ CANNOT DEPLOY', badge: 'DEPLOY' },
                { id: 'pipeline_diff', name: 'SCHEMA DIFF (阶段架构差异比对)', desc: isPipelineAdmin ? '自动比对各阶段模型架构、表字段变更及度量值元数据差异' : '需绑定部署管道以启用自动化架构差异比对检测引擎', statusClass: isPipelineAdmin ? 'enabled' : 'warn', statusText: isPipelineAdmin ? '✅ CAN COMPARE' : '⚠️ CANNOT COMPARE', badge: 'DIFF' }
            ];
            if (!hasSelectedWs) {
                colPipelineBody = `
                    <div style="padding: 16px 10px; text-align: center; background: rgba(255, 255, 255, 0.02); border-radius: 8px; border: 1px dashed rgba(255, 255, 255, 0.08);">
